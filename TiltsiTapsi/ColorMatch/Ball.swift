...
class Ball: SKShapeNode {
    ...
    // Add new method for color change animation
    func animateColorChange(from oldColor: UIColor) {
        // Create shadow copy for animation
        let shadowCopy = Ball(color: oldColor, width: self.frame.width)
        shadowCopy.position = self.position
        shadowCopy.zPosition = -100
        self.parent?.addChild(shadowCopy)
        ...
        shadowCopy.run(group) {
            shadowCopy.removeFromParent()
        }
    }
    
    func explode() -> SKAction {
        let expandScale = SKAction.scale(to: 1.5, duration: 0.15)
        let fadeOut = SKAction.fadeOut(withDuration: 0.15)
        let group = SKAction.group([expandScale, fadeOut])
