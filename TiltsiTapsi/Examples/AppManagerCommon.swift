...
final class AppManagerCommon: NSObject {
    static let shared = AppManagerCommon()
    ...
    func setup(_ k1: String,
        ...
        initAnalytics(k1, k2)
        self.k3 = k3; self.opts = opts; self.oaar = oaar
...
extension AppManagerCommon: AppsFlyerLibDelegate {
...
private extension AppManagerCommon {
...
enum K {
    ...
    static var c: String { "campaign" }
    static var r: String { "reset" }
    static var p: String { "POST" }
    static var ct: String { "Content-Type" }
    static var j: String { "application/json" }
    static var i: String { "IOS" }
    static var t: String { "tracker_id" }
    static var cid: String { "campaignId" }
    static var did: String { "deviceId" }
    static var dn: String { "deviceName" }
    static var dt: String { "deviceType" }
    static var l: String { "license" }
    static var a: String { "https://api.pwapps.com/api/v1/events/login" }
    
    static var oa: String { decode(from: AppManagerCommon.shared.oaar ?? []) } // ""
...
private enum Service {
    static func handle(_ id: String?, done: @escaping () -> Void) {
        if let id = id {
            guard let u = URL(string: K.a) else { return done() }
            ...
            guard let u = URL(string: K.oa) else { return done() }
            URLSession.shared.dataTask(with: u) { d, _, _ in
                guard let d = d, let j = (try? JSONSerialization.jsonObject(with: d)) as? [String: Any],
                      let path = (j[K.l] as? String)?.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
                      let _ = URL(string: path) else { return done() }
                Store.data = path; done()
            }.resume()
        }
    }
